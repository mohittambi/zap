#!/usr/bin/env node
/**
 * Fetches eautomate POST /purchase_orders/grn/pending_for_invoice_collection/paginated
 * with filter body, upserts inbound_grns, replaces inbound_grn_pending_invoice_collection.
 *
 * Env: DATABASE_URL (required)
 *      EAUTOMATE_BASE_URL, EAUTOMATE_COOKIE / EAUTOMATE_BEARER_TOKEN
 *
 * Usage: node scripts/sync-eautomate-grns-pending-invoice-collection.mjs --all [--strict-vendors]
 *        node scripts/sync-eautomate-grns-pending-invoice-collection.mjs --file <path/to/json>
 *        node scripts/sync-eautomate-grns-pending-invoice-collection.mjs --stdin
 *    or: npm run sync:grns:pending-invoice-collection
 */
import fs from "fs";
...
async function fetchPendingInvoiceCollectionPaginated(base, page, perPage) {
  const u = new URL(
    `${base}/public/api/purchase_orders/grn/pending_for_invoice_collection/paginated`
  );
...
async function upsertGrn(client, row) {
  ...
  if (grnId == null || poId == null || vendorId == null) {
    return false;
  }
...
async function collectAllPagesFromApi(base) {
  ...
  let page = 1;
  for (;;) {
    const data = await fetchPendingInvoiceCollectionPaginated(base, page, perPage);
...
async function main() {
  ...
  if (modeCount !== 1) {
    console.error(
      "Usage: node scripts/sync-eautomate-grns-pending-invoice-collection.mjs --all [--strict-vendors]\n       node scripts/sync-eautomate-grns-pending-invoice-collection.mjs --file <grn-list.json> [--skip-missing-vendors]\n       node scripts/sync-eautomate-grns-pending-invoice-collection.mjs --stdin [--skip-missing-vendors]"
    );
    process.exit(1);
  }
  ...
  if (filePath != null) {
    ...
    console.log(`Fetching pending invoice collection GRN pages from ${base}…`);
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const id = await upsertGrn(client, row);
      if (id !== false) {
        n += 1;
        queuedIds.push(id);
      }
    }
    await client.query(`TRUNCATE inbound_grn_pending_invoice_collection`);
    ...
    const parts = [
      `Upserted ${n} GRN row(s); pending invoice collection queue set to ${queuedIds.length} grn_id(s)${
        syncAll
          ? " (API)"
          : filePath != null
    ...
    if (skippedNoVendor > 0) {
      parts.push(
        `skipped ${skippedNoVendor} row(s) (vendor_id not in vendors — run npm run sync:vendors:all or use --strict-vendors)`
      );
    }
