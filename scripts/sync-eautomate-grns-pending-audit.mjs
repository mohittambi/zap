#!/usr/bin/env node
/**
 * Fetches eautomate POST /purchase_orders/grn/pending_for_audit/paginated,
 * upserts into inbound_grns, then replaces inbound_grn_pending_audit with those grn_ids.
 *
 * Env: DATABASE_URL (required)
 *      EAUTOMATE_BASE_URL, EAUTOMATE_COOKIE / EAUTOMATE_BEARER_TOKEN (same as sync-eautomate-grns.mjs)
 *
 * Usage: node scripts/sync-eautomate-grns-pending-audit.mjs --all [--strict-vendors]
 *        node scripts/sync-eautomate-grns-pending-audit.mjs --file <path/to/json>
 *        node scripts/sync-eautomate-grns-pending-audit.mjs --stdin
 *    or: npm run sync:grns:pending-audit
 */
import fs from "fs";
...
async function fetchPendingAuditPaginated(base, page, perPage) {
  const u = new URL(
    `${base}/public/api/purchase_orders/grn/pending_for_audit/paginated`
  );
...
async function collectAllPagesFromApi(base) {
  ...
  let page = 1;
  for (;;) {
    const data = await fetchPendingAuditPaginated(base, page, perPage);
...
async function main() {
  ...
  if (modeCount !== 1) {
    console.error(
      "Usage: node scripts/sync-eautomate-grns-pending-audit.mjs --all [--strict-vendors]\n       node scripts/sync-eautomate-grns-pending-audit.mjs --file <grn-list.json> [--skip-missing-vendors]\n       node scripts/sync-eautomate-grns-pending-audit.mjs --stdin [--skip-missing-vendors]"
    );
    process.exit(1);
  }
  ...
  if (filePath != null) {
    ...
    console.log(`Fetching pending-audit GRN pages from ${base}…`);
  ...
  try {
    ...
    const parts = [
      `Upserted ${n} GRN row(s); pending-audit queue set to ${queuedIds.length} grn_id(s)${
        syncAll
          ? " (API)"
          : filePath != null
