...
After a successful **`POST /api/inbound/grns/{grnId}/close`** (vendor invoice already on file), the server **best-effort** runs the same eligibility check as `POST …/debit-note`: if any accepted line has vendor `received_price` above `audit_price`, a **`DRAFT`** row is created in `inbound_zap_debit_notes`. If there is no discrepancy, terminal note state, or validation skip, the close response is unchanged. See [inbound-grn-debit-credit-note-flows.md](../../inbound-grn-debit-credit-note-flows.md) §1–2.
...
- **Pending audit** — rows in `inbound_grn_pending_audit` joined to `inbound_grns` (Zap DB is the source of truth for list membership as defined by this app).
