...
- Operators pack **per PO SKU** via **Enter packing** (modal) or the **Bulk form** (grid): each SKU can span **multiple physical boxes** (extra modal rows or bulk “Add box” per SKU). Apply updates the table; **Save lines** persists. The editor shows **PO Secondary SKU** and **Company Code Primary**. Optional TSV upload/download uses nine columns including explicit `box_number`.
