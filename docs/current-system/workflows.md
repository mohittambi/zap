...
sequenceDiagram
  ...
  API-->>UI: list
  Auditor->>UI: open one GRN → /inbound/grns/{grnId}
  UI->>API: GET /api/inbound/grns/{grnId}/details
  API->>DB: read header + snapshot + items + files
  loop for each line correction
    Auditor->>UI: enter accepted/rejected/shortage
  ...
  API->>DB: INSERT inbound_grn_logs (STATUS)
  API-->>UI: 200
  UI-->>Auditor: row gone — now in Pending Invoice Collection
```
...
| List route | [`src/app/api/inbound/pending-audits/grns/route.ts`](../../src/app/api/inbound/pending-audits/grns/route.ts) | GET pending list — `purchase_orders.read` |
| Status PATCH | [`src/app/api/inbound/grns/[grnId]/route.ts`](../../src/app/api/inbound/grns/[grnId]/route.ts) | PATCH `grn_audit_status='CLOSED'` — `purchase_orders.write` |
| Line PATCH | [`src/app/api/inbound/grns/[grnId]/items/[lineIndex]/route.ts`](../../src/app/api/inbound/grns/[grnId]/items/[lineIndex]/route.ts) | Quantity entry |
| Service | [`src/server/services/inboundGrnsService.ts`](../../src/server/services/inboundGrnsService.ts) | `listPendingAuditGrnsPaginated`, `updateGrnStatus`, `applyGrnQueueDelta` |
...
| Queue UI | `src/app/(app)/(logistics)/inbound/pending-audits/page.tsx` | List + Mark-audited |
