...
| `041` | `company_secondary_sku.company_code_primary`, `updated_at` |
| `042` | `zap_storage_path` on outbound PO files + inbound GRN invoice / DCN files (Supabase Storage) |
| `043` | `outbound_consignment_items`, `outbound_transporter_details`, `outbound_valid_box_names`; `outbound_consignments.detail_raw`, `detail_synced_at` |
| `044` | `outbound_po_logs` (PO activity from eAutomate) |
...
| `outbound_consignment_items`, `outbound_transporter_details`, `outbound_valid_box_names` | 043 |
| `outbound_po_logs` | 044 |
...
| `log_type` | VARCHAR(80) | Application-defined; includes audit controls: `AUDIT`, `AUDIT_DENIED`, `AUDIT_LOCKED`, `ACCOUNTS_DENIED`, `INVOICE_COLLECTION_DENIED`, `DCN_DECISION_DENIED`, plus `STATUS`, `LINE`, `DEBIT_NOTE`, `GRN` |
...
**`041`:** `company_code_primary` VARCHAR(50) (per-channel code), `updated_at` TIMESTAMPTZ.
...
**`042`:** `zap_storage_path` TEXT — when set, downloads serve from Supabase Storage instead of upstream proxy.
...
**`043`:** `detail_raw` JSONB, `detail_synced_at` TIMESTAMPTZ — richer consignment detail payload when synced.
...
### `outbound_consignment_items` (`043`)

Line items per consignment / PO from listings sync. FK `consignment_id` → `outbound_consignments` CASCADE. Columns include `po_number`, `po_secondary_sku`, company codes, `box_number`, `box_quantity`, `box_name`, demand/dispatch/consignment quantities, `mrp`, `raw` JSONB, `synced_at`. Indexes on `consignment_id`, `po_number`.

### `outbound_transporter_details` (`043`)

Transporter master from eAutomate. PK `id` (BIGINT), `name`, `raw` JSONB, `synced_at`.

### `outbound_valid_box_names` (`043`)

Valid box name list from eAutomate. SERIAL PK, `name` UNIQUE, `raw` JSONB, `synced_at`.

### `outbound_po_logs` (`044`)

Activity log lines per outbound PO. PK `id` (BIGINT). FK `outbound_po_id` → `outbound_purchase_orders` CASCADE. Columns: `po_number`, `consignment_id`, `foreign_key`, `operation`, `remarks`, `created_by`, `created_at`, `raw` JSONB, `synced_at`. Indexes on `outbound_po_id`, `created_at`.

### Inbound file tables — `zap_storage_path` (`042`)
