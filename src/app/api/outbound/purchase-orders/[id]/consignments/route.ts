...
/** Create a consignment directly in Zap (PO must be WIP). */
  ...
  try {
    ...
    if (wip !== "YES") {
      return NextResponse.json(
        { error: "PO must be marked WIP before creating a consignment." },
        { status: 400 }
      );
    }

    const consignment = await createOutboundConsignmentInZap(po, user.email);
